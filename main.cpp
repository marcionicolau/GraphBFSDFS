...
template <class ListType>//type of data each vertex stores
class Graph
{
private:
    int max;
    ...
    {
        vertex *vertexPtr;//points to the vertex to which the edge is adjacent
    ...
    //accepts reference to pointer and pointer to vertex
...
public:
    ...
    //set edges between vertices vert1 and vert2 by iterating
    {
...
//    Graph <char>g2(value,size);
    Graph <std::string>g1(value,size);
    Graph <std::string>g2(value,size);
    int choice = 1;
//    char vertA, vertB, searchElement;
    std::string vertA, vertB, searchElement;
    while(1)
    {
